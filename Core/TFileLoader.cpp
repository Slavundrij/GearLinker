...
map<UnicodeString, map<double, pair<double, double> > > RollerSizes = {
    { gears_id[0], { { 9.822, pair(0.059, 1) }, { 9.793, pair(0.058, 0.99) },
                       { 10.035, pair(0.056, 1.074) } } },
    { gears_id[1], { { 36.445, pair(0.076, 1) }, { 36.409, pair(0.077, 0.99) },
                       { 36.715, pair(0.071, 1.074) } } },
    { gears_id[2], { { 11.099, pair(0.043, 1) }, { 11.068, pair(0.044, 0.99) },
                       { 11.325, pair(0.041, 1.074) } } },
    { gears_id[3], { { 35.203, pair(0.072, 1) }, { 35.168, pair(0.072, 0.99) },
                       { 35.463, pair(0.07, 1.074) } } },
    { gears_id[4], { { 9.822, pair(0.045, 1) }, { 9.793, pair(0.045, 0.99) },
                       { 10.035, pair(0.043, 1.074) } } },
    { gears_id[5], { { 36.445, pair(0.067, 1) }, { 36.409, pair(0.067, 0.99) },
                       { 36.715, pair(0.065, 1.074) } } },
    ...
    TList* goodGearList, AnsiString FileName, TMemo* memoLog, TMemo* memoInfo)
{
    ...
    try {
        ...
        ExcelBook = ExcelApp.OlePropertyGet("Workbooks")
                        .OleFunction("Open",
                            WideString(FileName)); // Open Excel File.
        ExcelSheet =
            ExcelBook.OlePropertyGet("WorkSheets").OlePropertyGet("Item", 1);
        memoInfo->Lines->Add(
            Format("Обрабатываю файл %s", WideString(FileName)));

        int iRowsCount = ExcelApp.OlePropertyGet("ActiveSheet")
                             .OlePropertyGet("UsedRange")
                             .OlePropertyGet("Rows")
        ...
        for (int i = 15; i < iColsCount; i++) {
            ...
            int color = vCells.OlePropertyGet("Item", 9, i)
                            .OlePropertyGet("Interior")
                            .OlePropertyGet("ColorIndex");
            // && color != 6
            if (isCellFilled(gearNumber) && color != 6) {
                TGear* newGear =
                    new TGear(PGTS, orderNum, sGearName, gearNumber);
                ...
                switch (fillGearMeasurments(
                    &filledMeasuarments, newGear, listparams, i)) {
                    case 0:
        ...
        memoInfo->Lines->Add(Format(
            "Найдено %d деталей %s", quantity, PGTS.getStringDesignation()));
        memoInfo->Lines->Add("");
    } catch (Exception &E) {
    ...
    TFileList* FileList, TMemo* memoRes)
{
    ...
    buildStandartGearboxes(
        stanGearList, UsedGearList, memoLog, memoInfo, memoRes);
    ...
    for (int i = 0; i < FileList->iGetCountFiles(); i++) {
        ...
        try {
            ExcelBook = ExcelApp.OlePropertyGet("Workbooks")
                            .OleFunction("Open",
                                WideString(FileName)); // Open Excel File.
            ExcelSheet = ExcelBook.OlePropertyGet("WorkSheets")
...
void findAllPairs(
    TList* gearList, TGear* Gear, TList* FindMatches, int num, TMemo* memoLog)
{
    int m = gearWheel(num);
        ...
        if (Wheel->Desgination.getStringDesignation() == gears_id[m]) {
            ...
            if (D <= tolerance && D >= tolerance - Wheel->extTol) {
                ...
                memoLog->Lines->Add(Format(
                    "Найдено зацепление %s+%s", Gear->sNumber, Wheel->sNumber));
            }
        }
    }
    ...
    TMemo* memoLog, TMemo* memoInfo, TMemo* memoRes)
{
    ...
    for (int i = 0; i < gears_id.size() / 2; i++) {
        ...
        memoLog->Lines->Add("++++++++++++++++++");
        memoLog->Lines->Add("");
        memoLog->Lines->Add(Format("Далее идут все найденные зацепления %s+%s",
            gears_id[i * 2], gears_id[i * 2 + 1]));
        memoLog->Lines->Add("");
        memoLog->Lines->Add("++++++++++++++++++");
        while (1) {
        ...
        memoLog->Lines->Add("++++++++++++++++++");
        memoLog->Lines->Add("");
        memoLog->Lines->Add(Format("Далее идут сборки по критерию %s+%s",
            gears_id[i * 2], gears_id[i * 2 + 1]));
        memoLog->Lines->Add("");
        memoLog->Lines->Add("++++++++++++++++++");
        ...
        Gears_and_Wheels_3.size() != 0 && Gears_and_Wheels_4.size() != 0)
    {
        ...
        auto t = time(nullptr);
        auto tm = *localtime(&t);
		o->AddPair(new TJSONPair("Date",
            Format("%d.%d.%d", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900)));
        o->AddPair("Gearboxes", Gearboxes);
        while (Gears_and_Wheels_1.size() != 0 &&
               Gears_and_Wheels_2.size() != 0 &&
			   Gears_and_Wheels_3.size() != 0 &&
			   Gears_and_Wheels_4.size() != 0)
        {
            gearboxN++;
            ...
            Gears_and_Wheels_1.erase(
                Gears_and_Wheels_1.begin(), Gears_and_Wheels_1.begin() + 2);
            Gears_and_Wheels_2.erase(
                Gears_and_Wheels_2.begin(), Gears_and_Wheels_2.begin() + 2);
            Gears_and_Wheels_3.erase(
                Gears_and_Wheels_3.begin(), Gears_and_Wheels_3.begin() + 2);
            Gears_and_Wheels_4.erase(
                Gears_and_Wheels_4.begin(), Gears_and_Wheels_4.begin() + 2);
        }

        remove("DB65.json");

		if (FormAddDataFiles->SaveDialog1->Execute()) {
            auto_ptr<TStreamWriter> DestFileStream(new TStreamWriter(
                FormAddDataFiles->SaveDialog1->FileName + ".json",
                TEncoding::UTF8));
        ...
        vCells1.OlePropertyGet("Item", 1, 1)
            .OlePropertySet("Value", WideString("Двигатель"));
        vCells1.OlePropertyGet("Item", 1, 2)
            .OlePropertySet("Value", WideString("Шестерня ведущая"));
        vCells1.OlePropertyGet("Item", 1, 3)
            .OlePropertySet("Value", WideString("ПГТС.33811.022"));

        for (int i = 0; i < names.size() / 8; i++) {
            vCells1.OlePropertyGet("Item", i + 2, 2)
                .OlePropertySet("Value", WideString(names.at(i * 8)));
        }
        ...
        vCells1.OlePropertyGet("Item", 1, 1)
            .OlePropertySet("Value", WideString("Колесо промежуточное"));
        vCells1.OlePropertyGet("Item", 1, 2)
            .OlePropertySet("Value", WideString("Шестерня промежуточная"));
        vCells1.OlePropertyGet("Item", 1, 3)
            .OlePropertySet("Value", WideString("ПГТС.33811.015"));
        for (int i = 0; i < names.size() / 8; i++) {
            vCells1.OlePropertyGet("Item", i + 2, 1)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 1)));
            vCells1.OlePropertyGet("Item", i + 2, 2)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 2)));
        }
        vCells1.OlePropertySet("ColumnWidth", 30);
        ...
        vCells1.OlePropertyGet("Item", 1, 2)
            .OlePropertySet("Value", WideString("Шестерня промежуточная"));
        vCells1.OlePropertyGet("Item", 1, 3)
            .OlePropertySet("Value", WideString("ПГТС.33811.016"));
        for (int i = 0; i < names.size() / 8; i++) {
            vCells1.OlePropertyGet("Item", i + 2, 1)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 3)));
            vCells1.OlePropertyGet("Item", i + 2, 2)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 4)));
        }
        vCells1.OlePropertySet("ColumnWidth", 30);
        ...
        vCells1.OlePropertyGet("Item", 1, 2)
            .OlePropertySet("Value", WideString("Шестерня промежуточная"));
        vCells1.OlePropertyGet("Item", 1, 3)
            .OlePropertySet("Value", WideString("ПГТС.33811.014"));
        for (int i = 0; i < names.size() / 8; i++) {
            vCells1.OlePropertyGet("Item", i + 2, 1)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 5)));
            vCells1.OlePropertyGet("Item", i + 2, 2)
                .OlePropertySet("Value", WideString(names.at(i * 8 + 6)));
        }
        vCells1.OlePropertySet("ColumnWidth", 30);
        ...
        bool firstTime = true;
        vCells1.OlePropertyGet("Item", 1, 1)
            .OlePropertySet("ColumnWidth", 16.55);
        vCells1.OlePropertyGet("Item", 1, 2)
            .OlePropertySet("ColumnWidth", 15.27);
        for (int i = 0; i < names.size() / 8; i++) {
            colNum = i % 7;
            if (i % 7 == 0) {
                if (!firstTime) {
                ...
                vCells1.OlePropertyGet("Item", rowNum + 1, 1)
                    .OlePropertySet("Value", WideString("Вал выходной"));

                string range =
                    "A" + to_string(rowNum + 2) + ":A" + to_string(rowNum + 3);
                Variant cellRange = ExcelSheet1.OlePropertyGet(
                    "Range", WideString(range.c_str()));
                cellRange.OleProcedure("Merge");
                cellRange.OlePropertySet(
                    "Value", WideString("Блок шестерен 017"));
                range =
                    "A" + to_string(rowNum + 4) + ":A" + to_string(rowNum + 5);
                cellRange = ExcelSheet1.OlePropertyGet(
                    "Range", WideString(range.c_str()));
                cellRange.OleProcedure("Merge");
                cellRange.OlePropertySet(
                    "Value", WideString("Блок шестерен 016"));
                range =
                    "A" + to_string(rowNum + 6) + ":A" + to_string(rowNum + 7);
                cellRange = ExcelSheet1.OlePropertyGet(
                    "Range", WideString(range.c_str()));
                cellRange.OleProcedure("Merge");
                cellRange.OlePropertySet(
                    "Value", WideString("Блок шестерен 015"));
                vCells1.OlePropertyGet("Item", rowNum + 8, 1)
                    .OlePropertySet("Value", WideString("Ведущая шестерня"));

                vCells1.OlePropertyGet("Item", rowNum + 1, 2)
                    .OlePropertySet("Value", WideString(gears_id[7]));
                vCells1.OlePropertyGet("Item", rowNum + 2, 2)
                    .OlePropertySet("Value", WideString(gears_id[6]));
                vCells1.OlePropertyGet("Item", rowNum + 3, 2)
                    .OlePropertySet("Value", WideString(gears_id[5]));
                vCells1.OlePropertyGet("Item", rowNum + 4, 2)
                    .OlePropertySet("Value", WideString(gears_id[4]));
                vCells1.OlePropertyGet("Item", rowNum + 5, 2)
                    .OlePropertySet("Value", WideString(gears_id[3]));
                vCells1.OlePropertyGet("Item", rowNum + 6, 2)
                    .OlePropertySet("Value", WideString(gears_id[2]));
                vCells1.OlePropertyGet("Item", rowNum + 7, 2)
                    .OlePropertySet("Value", WideString(gears_id[1]));
                vCells1.OlePropertyGet("Item", rowNum + 8, 2)
                    .OlePropertySet("Value", WideString(gears_id[0]));
            }
            vCells1.OlePropertyGet("Item", rowNum, colNum + 3)
                .OlePropertySet(
                    "Value", WideString(Format("Редуктор %d", i + 1)));
            vCells1.OlePropertyGet("Item", rowNum, colNum + 3)
                .OlePropertySet("ColumnWidth", 14.73);
            for (int o = 7; o > -1; o--) {
                vCells1
                    .OlePropertyGet("Item", rowNum + abs(o - 7) + 1, colNum + 3)
                    .OlePropertySet("Value", WideString(names.at(o + i * 8)));
            }
        }
    }
    int minCol = 30;
    ...
    memoRes->Lines->Add(
        Format("Для дальнейшей сборки редукторов не хватает %d деталей %s и %s",
            secMinCol - minCol, gears_id[minNum], gears_id[minNum + 1]));
...
int fillGearMeasurments(
    vector<unsigned int>* measurements, TGear* Gear, TList* listparams, int col)
{
    try {
        Variant MYCells = ExcelApp.OlePropertyGet("Cells");
        for (int i = 0; i < measurements->size(); i++) {
            ...
            UnicodeString nameStr =
                MYCells.OlePropertyGet("Item", measurements->at(i), 2);
            nameStr.Trim();
            if (!cellIsMeasure(nameStr)) {
                Measurment->sKeyMeasure = "Окончательный контроль";
                (currStr == "OK") ? Measurment->fMeasure = 1
                ...
                currStr =
                    MYCells.OlePropertyGet("Item", measurements->at(i), 11);
                try {
                ...
                if (Measurment->sKeyMeasure == baseMeasure) {
                    map<double, pair<double, double> > Nominal_to_all =
                        RollerSizes[Gear->Desgination.getStringDesignation()];
                    if (Nominal_to_all.find(Measurment->fNominalValue) !=
                        Nominal_to_all.end()) {
                        Gear->extTol =
                            Nominal_to_all[Measurment->fNominalValue].first;
                        Gear->rollerD =
                            Nominal_to_all[Measurment->fNominalValue].second;
                    } else {
                    }
                }
                currStr =
                    MYCells.OlePropertyGet("Item", measurements->at(i), 12);
                try {
                ...
                currStr =
                    MYCells.OlePropertyGet("Item", measurements->at(i), 13);
                try {
...
bool measureInLimits(stMeasurement* Measurment)
{
    if (Measurment->sKeyMeasure == "9.9.") {
        if (Measurment->fMeasure >= 0.09)
            return false;
        else
            return true;
    } else {
        if (Measurment->fMeasure >=
                Measurment->fNominalValue + Measurment->fToleranceDown &&
            Measurment->fMeasure <=
